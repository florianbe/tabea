...
@section('sidebar')
    @include('substudies.sidebars.overview', ['studyId' => $questiongroup->substudy->study->id, 'hasAccess' => Auth::user()->hasAccessToStudy($questiongroup->substudy->study), 'canContribute' => (Auth::user()->isAdmin || $questiongroup->substudy->study->contributors->contains(Auth::user()))])
@stop
@section('content')
            ...
            <div class="row">
                <div  class="form-group">
                    <div class="col-md-4 typeoption numeric slider">
                        {{ Bootstrap::number('min_numeric', trans('pagestrings.question_min_numeric')) }}
                        {{ show_errors_for('min_numeric', $errors) }}
                    </div>
                    <div class="col-md-4 typeoption numeric slider">
                        {{ Bootstrap::number('max_numeric', trans('pagestrings.question_max_numeric')) }}
                        {{ show_errors_for('max_numeric', $errors) }}
                    </div>
                    <div class="col-md-4 typeoption slider">
                        {{ Bootstrap::number('step_numeric', trans('pagestrings.question_step_numeric')) }}
